...
SUITE(TestFolderMonitor) {
	...
	TEST_FIXTURE(SimpleMonitor, DetectAddDelete) {
		...
		Sleep(100);
		CHECK(CopyFile(
			(g_datapath + TX("\\Folder\\a\\file1.txt")).c_str(),
			(g_datapath + TX("\\Folder\\a\\temp.txt")).c_str(),
			false));
		CHECK(DeleteFile((g_datapath + TX("\\Folder\\a\\temp.txt")).c_str()));
		Sleep(100);
		mon.Close();
		CHECK_EQUAL(2, events.size());
		if(events.size() < 2) return;
		for(int i=0;i<2;++i) {
			CHECK(TX("temp.txt") == events[i].Entry.Name);
			CHECK_EQUAL(TypeFile, events[i].Entry.Type);
			CHECK(g_datapath + TX("\\Folder\\a\\") == events[i].Path);
		}
