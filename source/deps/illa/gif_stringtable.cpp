...
namespace Img {
	...
	{
		if (minCodeSize > 11) {
			DO_THROW(Err::InvalidParam, TX("StringTable::ctor: Invalid initial code size."));
		}
	...
	short StringTable::addString(short lastCode, short currentCode) {
		...
		if (m_numCodes > 4095) {
			DO_THROW(Err::CodecError, TX("Code table size out of bounds."));
		}

		if (lastCode < 0 || currentCode < 0) {
			DO_THROW(Err::CodecError, TX("LZW stream invalid."));
		}
