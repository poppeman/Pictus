	...
	namespace Internal {
		void BMPHeader::Load(IO::FileReader::Ptr file) {
			...
			if ((biSize != 12) && (biSize != 40) && (biSize != 56) && (biSize != 108) && (biSize != 124)) {
				DO_THROW(Err::InvalidHeader, "Header size mismatch.");
			}
			...
			if (planes != 1) {
				DO_THROW(Err::InvalidHeader, "Invalid number of planes; must equal 1.");
			}
			...
			if (biSize == 12) {
				...
				if ((ColorDepth == 1) && (Compression != CompressRGB)) {
					DO_THROW(Err::InvalidHeader, "Monochrome bitmaps must be uncompressed.");
				}

				// 4-bit
				if ((Compression != CompressRGB) && (Compression != CompressRLE4) && (ColorDepth == 4)) {
					DO_THROW(Err::InvalidHeader, "Invalid compression for 4-bit bitmap format");
				}

				// 8-bit
				if ((Compression != CompressRGB) && (Compression != CompressRLE8) && (ColorDepth == 8)) {
					DO_THROW(Err::InvalidHeader, "Invalid compression for 8-bit bitmap format");
				}

				// 16-bit
				if ((ColorDepth == 16) && (Compression != CompressRGB) && (Compression != CompressBitfields)) {
					DO_THROW(Err::InvalidHeader, "Invalid compression for 16-bit bitmap format");
				}
				// 24-bit
				if ((Compression != CompressRGB) && (ColorDepth == 24)) {
					DO_THROW(Err::InvalidHeader, "Invalid compression for 24-bit bitmap format");
				}

				// 32-bit
				if ((ColorDepth == 32) && (Compression != CompressRGB) && (Compression != CompressBitfields)) {
					DO_THROW(Err::InvalidHeader, "Invalid compression for 32-bit bitmap format");
				}
		...
		uint32_t BMPHeader::calcShift(uint32_t mask) {
			if (mask == 0) {
				DO_THROW(Err::InvalidHeader, "BMP header contained invalid color masks.");
			}
