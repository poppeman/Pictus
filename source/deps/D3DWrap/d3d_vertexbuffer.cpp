...
namespace D3D {
	...
	{
		if (vb == 0) {
			DO_THROW(Err::CriticalError, "Vertex buffer not initialized");
		}
	}

	uint8_t* VertexBuffer::Lock() {
		if (m_isLocked) {
			DO_THROW(Err::CriticalError, "Vertex buffer is already locked.");
		}
		void* buf;
		m_vb->Lock(0, 0, &buf, 0);
		m_isLocked = true;
	...
	void VertexBuffer::Unlock() {
		if (m_isLocked == false) {
			DO_THROW(Err::CriticalError, "Vertex buffer is not locked.");
		}
		m_vb->Unlock();
		m_isLocked = false;
	}
