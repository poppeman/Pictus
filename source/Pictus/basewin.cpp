...
namespace Win {
	...
	void BaseWindow::Caption(const std::string &newcapt) {
		if (Handle()) SetWindowText(Handle(), UTF8ToWString(newcapt).c_str());
	}

	void BaseWindow::Caption(const int sidCaption) {
		m_sidCaption = sidCaption;
		Caption(Intl::GetString(m_sidCaption));
	}

	const std::string BaseWindow::Caption() const {
		if (Handle()) {
			// SEXY!
	...
	std::string BaseWindow::OpenFileDialog(const std::string& title, const std::string& filters, size_t defaultFilterIndex) {
		const int MaxPath = 4096;
		wchar_t filename[MaxPath] = L"";

		auto wFilters = UTF8ToWString(filters);
		auto wTitle = UTF8ToWString(title);
		...
		SetCurrentDirectoryW(UTF8ToWString(IO::GetPath(Sys::Info::ExePath())).c_str());
	...
	std::string BaseWindow::m_exceptionDescription;
...

		{
			...
			if (pWin != 0) {
				...
				try {
					...
					pWin->m_exceptionDescription = "Unknown Exception";
					pWin->m_exceptionOcurred = true;
					PostQuitMessage(0);
				}
			}
	...
	std::string BaseWindow::UnhandledExceptionDescription() {
