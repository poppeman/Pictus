...
namespace Win {
	...
	BaseWindow::BaseWindow()
		...
		m_sidCaption(-1)
	{
		m_lang = Intl::OnLanguageChanged.connect([&]() { UpdateCaption(); });
	}
	...
	bool BaseWindow::OnClose() { return PerformOnClose(); }
	bool BaseWindow::OnPaint() { return PerformOnPaint(); }
	bool BaseWindow::OnEraseBackground(HDC hdc)  { return PerformOnEraseBackground(hdc); }
	bool BaseWindow::OnContextMenu(const PointInt& pt)  { return PerformOnContextMenu(pt); }
	bool BaseWindow::Onchar(uint32_t key, int count, bool pressed) { return PerformOnchar(key, count, pressed); }
	...
	bool BaseWindow::OnCopyData(const COPYDATASTRUCT* pcds)  { return PerformOnCopyData(pcds); }
	void BaseWindow::OnCreateContextMenu(HMENU hMenu) { PerformOnCreateContextMenu(hMenu); }
	...
	LRESULT BaseWindow::baseWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, bool doDefProc) {
		if ((msg >= WM_APP) && (msg < WM_APP + 16))
			if (OnApp(msg - WM_APP, wParam, lParam)) return 0;
