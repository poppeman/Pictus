...
namespace Win {
	...
	void WinRenderTarget::Render( const Geom::PointInt& sourceZoomedTopLeft, const Img::Properties& props ) {
		if (m_hwnd == nullptr) {
			DO_THROW(Err::CriticalError, "TargetWindow not set");
		}
		if (m_renderer == nullptr) {
			DO_THROW(Err::CriticalError, "Renderer not set");
		}
		if (m_redrawStrategy == nullptr) {
			DO_THROW(Err::CriticalError, "Redraw strategy not set");
		}
	...
	Img::Surface::Ptr WinRenderTarget::CreateSurface() {
		if (m_renderer == nullptr) {
			DO_THROW(Err::CriticalError, "Renderer not yet set.");
		}
